...
table = {
    ...
    'ROC / MCA Annual Compliance': f'{rupee}18,000',
...
for filepath in html_files:
    ...
    for name in table:
        ...
        if name == 'Startup India DPIIT Audit' and 'startup' in filepath.lower():
            matched_name = name
            break
        if 'iso27001' in filepath.lower() or 'information-security' in filepath.lower():
            matched_name = 'Information Security Gap Assessment'
            break
    ...
    if matched_name:
        ...
        # In hero price (for AI services) they specifically requested 'excluding taxes'
        # "just write $707 excluding taxes in the service page" - we apply this to AI tools
        if 'ai-' in filepath.lower() and '<div class="hero-price">' in content:
        ...
        # Replaces svc-meta-val e.g. "Starting From ₹50,000 / assessment"
        def replace_meta(m):
            text = m.group(0)
            suffix = ''
            ...
            # The AI pages use 'excluding taxes' in hero, not here (or they do here?)
            # The user complained about vextintel-global having 'excluding taxes'. So we don't append it globally.
        ...
        # Replace sidebar price (usually an absolute amount without suffix)
